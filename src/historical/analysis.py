...
from .orderbook import seconds_to_time
from .schema import OrderbookSnapshot, TickerSnapshots
...
def snapshot_mid_price(snap: OrderbookSnapshot) -> float | None:
    """
...
def align_mid_price_series(
    books: list[TickerSnapshots],
) -> tuple[list[str], dict[str, list[float | None]]]:
    """
    Given a list of `TickerSnapshots`, return `(time_grid, mid_by_ticker)`.

    `time_grid` is a list of "HH:MM:SS" strings at 1-second resolution
    covering the union of the tickers' active windows. `mid_by_ticker[ticker]`
    is a list of mid-prices aligned to `time_grid` — `None` before that
    ticker's first event and after its last event; forward-filled in between.
    """
    if not books:
        return [], {}
...
def mid_price_dataframe(books: list[TickerSnapshots]) -> pd.DataFrame:
...
def write_mid_price_csv(books: list[TickerSnapshots], output_path: str | Path) -> int:
